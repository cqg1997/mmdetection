...
def init_weights(self):
    ...
    for _, filter in enumerate(self.upsample_filter):
        constant_init(filter, 0, 1)
    for _, filter in enumerate(self.downsample_filter):
        constant_init(filter, 0, 1)
...
def forward(self, inputs):
    ...
    for i in range(used_backbone_levels - 1, 0, -1):
        ...
        switch = self.upsample_filter[i-1](up_feat)

        laterals[i - 1] = switch*up_feat + (1-switch)*laterals[i - 1]
    ...
    for i in range(0, used_backbone_levels - 1):
        down_feat = self.downsample_convs[i](inter_outs[i])
        switch = self.downsample_filter[i](down_feat)
        inter_outs[i + 1] = switch* down_feat + (1-switch) * inter_outs[i+1]
